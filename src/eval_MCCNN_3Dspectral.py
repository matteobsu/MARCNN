...
if __name__ == '__main__':
    ...
    #==========================================================================#
    numbers = range(7990,8050,1)
    name_filter = []
    for num in numbers:
        name_filter.append(str(num))
    inputFileArray = []
    ...
    if argDict["dataPath"]==None:
        ...
        for name in glob.glob(os.path.join(argDict["dataPath"],'*X*.h5')):
            if any(num_str in name for num_str in name_filter):
    ...
    #else:
    #    for name in glob.glob(os.path.join("./input",'*X*.h5')):
    #        inputFileArrayTemp.append(name)
    #    for name in glob.glob(os.path.join("./input",'*Y*.h5')):
    ...
    otype = None
    dumpDataFile = h5py.File(scatterFileArray[0], 'r')
    ...
    with sess.as_default():
        ...
        transform_shape_in = (96, 96, shape_process_in[2], shape_process_in[3])
        transform_shape_scatter = (96, 96, shape_process_out[2], shape_process_out[3])
        transform_shape_out = (96, 96, shape_process_out[2], shape_process_out[3])
        ...
        for imagenr in itertools.islice(itertools.count(), 0, lenX):
            ...
            dot_pos = observationFileArray[imagenr].rfind(".")
            slash_pos = observationFileArray[imagenr].rfind("/")
            basename = observationFileArray[imagenr][slash_pos+1:dot_pos]

            file = h5py.File(inputFileArray[imagenr], 'r')
            inImage = numpy.array(file['data']['value'])
            file.close()
            predictIn = numpy.array(inImage)
            # non-norm, non-resize input
            ...
            predictIn = transform.resize(predictIn, (slice_size[0], slice_size[1], channelNum_in), order=3,
                                         mode='reflect')
            predictIn = predictIn.reshape(transform_shape_in)
            minValX, maxValX, predictIn = normaliseFieldArray(predictIn, channelNum_in, flatField, input_type)
            predictIn = predictIn.reshape((1,) + predictIn.shape)
            predictIn = predictIn.astype(numpy.float32)

            file = h5py.File(observationFileArray[imagenr], 'r')
            outImage = numpy.array(file['data']['value'])
            file.close()
            out_imsize = (outImage.shape[0], outImage.shape[1])
            outImage = transform.resize(outImage, (slice_size[0], slice_size[1], outImage.shape[2]), order=3,
                                        mode='reflect')
            outImage = outImage.reshape(transform_shape_out)
            minValZ, maxValZ, outImage = normaliseFieldArray(outImage, channelNum_out, flatField, ref_type)
            outImage = outImage.reshape((1,) + outImage.shape)
            outImage = outImage.astype(numpy.float32)

            file = h5py.File(scatterFileArray[imagenr], 'r')
            scatterImage = numpy.array(file['data']['value'])
            file.close()
            scatterImage = transform.resize(scatterImage, (slice_size[0], slice_size[1], scatterImage.shape[2]),
                                            order=3, mode='reflect')
            scatterImage = scatterImage.reshape(transform_shape_scatter)
            minValY, maxValY, scatterImage = normaliseFieldArray(scatterImage, channelNum_out, flatField, output_type)
            scatterImage = scatterImage.reshape((1,) + scatterImage.shape)
            scatterImage = scatterImage.astype(numpy.float32)
            # ==========================================================================================================#
            ...
            if fullRun == False:
                plt.subplot(151)
                plt.imshow(predictIn[0, :, :, 0, 0].squeeze(), cmap='gray')
                plt.title("Scattered In.")
            ...
            if fullRun == False:
                plt.subplot(152)
                plt.imshow(img[0, :, :, 0, 0].squeeze(), cmap='gray')
                plt.title("Scatter Pred.")
            ...
            if fullRun == False:
                plt.subplot(153)
                plt.imshow(scatterImage[0, :, :, 0, 0].squeeze(), cmap='gray')
                plt.title("Scatter GT")
            # imgArr[imagenr,2,:,:,:] = outImage.squeeze()
            scatterImage = scatterImage.astype(otype)
            scatterImage = denormaliseFieldArray(scatterImage[0], channelNum_out, minValY, maxValY, flatField,
                                                 output_type)
            imgArr[imagenr, 2, :, :, :] = scatterImage[:, :, :, 0]

            # scatter-corrected prediction #
            # correctedImage = inImage.squeeze()-img.squeeze()
            correctedImage = predictIn - img
            minValZ, maxValZ, correctedImage = normaliseFieldArray(correctedImage, channelNum_out, flatField, ref_type)
            correctedImage = correctedImage.astype(numpy.float32)

            if fullRun == False:
                plt.subplot(154)
                # plt.imshow(correctedImage[0,:,:,0].squeeze(), cmap='gray')
                plt.imshow(correctedImage[:, :, 0, 0].squeeze(), cmap='gray')
                plt.title("Corrected Pred.")
            # imgArr[imagenr,3,:,:,:] = correctedImage.squeeze()
            correctedImage = correctedImage.astype(otype)
            correctedImage = denormaliseFieldArray(correctedImage, channelNum_out, minValZ, maxValZ, flatField,
                                                   ref_type)
            imgArr[imagenr, 3, :, :, :] = correctedImage[:, :, :, 0]

            # ============================= #
            # Get full-scale image predict. #
            # ============================= #
            pred_scatterImg = transform.resize(img[:, :, :, 0], (out_imsize[0], out_imsize[1], channelNum_in), order=3,
                                               mode='reflect')
            pred_outImg = inImage - pred_scatterImg

            # scatter-corrected groundtruth #
            if fullRun == False:
                plt.subplot(155)
                plt.imshow(outImage[0, :, :, 0, 0].squeeze(), cmap='gray')
                plt.title("Corrected GT")
            # imgArr[imagenr,4,:,:,:] = outImage.squeeze()
            outImage = outImage.astype(otype)
            outImage = denormaliseFieldArray(outImage[0], channelNum_out, minValZ, maxValZ, flatField, ref_type)
            imgArr[imagenr, 4, :, :, :] = outImage[:, :, :, 0]

            h5name = os.path.join(outPath, basename + ".h5")
            result_file = h5py.File(h5name, 'w')
            dgroup = result_file.create_group("data")
            dgroup.create_dataset("value", data=pred_outImg)
            result_file.close()

            # errArr[imagenr,:,:] = numpy.square(outImage.squeeze() - img.squeeze())
            # errArr[imagenr,:,:,:] = numpy.square(outImage.squeeze() - img.squeeze())
            errArr[imagenr, :, :, :] = (outImage - correctedImage)[:, :, :, 0]
            ...
            scatterErr = (scatterImage - img)[:, :, :, 0]
            normScatterErr = None
            if channelNum_out <= 1:
                normScatterErr = numpy_normalize(scatterErr).astype(numpy.float32)
            else:
                normScatterErr = numpy.zeros((scatterErr.shape[0], scatterErr.shape[1], scatterErr.shape[2]),
                                             dtype=numpy.float32)
                for channelIdx in itertools.islice(itertools.count(), 0, channelNum_out):
                    normScatterErr[:, :, channelIdx] = numpy_normalize(scatterErr[:, :, channelIdx]).astype(
                        numpy.float32)
            scatterErr = numpy.square(scatterErr)
            scatterError = numpy.mean(scatterErr)
            normScatterErr = numpy.square(normScatterErr)
            normScatterError = numpy.mean(normScatterErr)
            print("MSE scattermap img %d: %g" % (imagenr, scatterError))
            print("normalized MSE scattermap img %d: %g" % (imagenr, normScatterError))
            ...
            if fullRun:
                for channelIdx in itertools.islice(itertools.count(), 0, channelNum_in):
                    ...
                    plt.imshow(scatterImage[:, :, channelIdx, 0].squeeze(), cmap='gray')
                    # figs[channelIdx].figimage(scatterImage[0,:,:,channelIdx].squeeze(), cmap='gray')
                    plt.title("Scatter GT")
                    # figs[channelIdx].suptitle("Scatter GT")
                    plt.subplot(154)
                    # figs[channelIdx].add_subplot(154)
                    plt.imshow(correctedImage[:, :, channelIdx, 0].squeeze(), cmap='gray')
                    # figs[channelIdx].figimage(correctedImage[:,:,channelIdx].squeeze(), cmap='gray')
                    plt.title("Corrected Pred.")
                    # figs[channelIdx].suptitle("Corrected Pred.")
                    plt.subplot(155)
                    # figs[channelIdx].add_subplot(155)
                    plt.imshow(outImage[:, :, channelIdx, 0].squeeze(), cmap='gray')
                    # figs[channelIdx].figimage(outImage[0,:,:,channelIdx].squeeze(), cmap='gray')
                    plt.title("Corrected GT")
                    # figs[channelIdx].suptitle("Corrected GT")
